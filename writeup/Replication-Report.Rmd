...
I plan to run 50 participants, native speakers of English. This status will be determined by asking each participant to enter their native language, and excluding those participants who enter something other than English. Since it is a text-box, the data may require cleanup to correct for obvious typos before processing. 
...
for (f in files) {
  ...
  id <- data.frame(subid = f,
                   ...
                   screen_size = addnas(jd$answer$data$screen_size))
...
```{r}
#The mean zone for ezch adjective -- equivalent to Table 1
d %>%
  group_by(verb, adj) %>%
...
![Table 1 from Tribushinina, compring the mean number of items selected for each adjective](figures/trib-tab-1-mean-zones.png)
...
#create graph equivalent to figure 2 -- mena number of items labelled for each adjective for each prototypicality status
