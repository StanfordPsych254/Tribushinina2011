...
d <- filter(d.raw, prototype_status != "na") %>%
  select(-language) %>%
  group_by(subid) %>%
  summarise(perc_non_consec = sum(non_consec)/num_trials, perc_no_enpoint = (length(endpoint)-sum(endpoint))/num_trials)%>%
  
#need to check for having more than 10% non_consecutive or more than 10% with no endpoint, and exclude those participants, and then exclude any remaining trials in which there is non_consecutive data or no endpoint.  
filter(perc_no_enpoint > .1) %>%
filter(perc_non_consec > .1)

head(d)
```
```{r}
#exclude participants with more than 10% non-consecutive answers or more than 10% answers with no endpoint


#exclude trials with non consecutive answers, and without endpoints
```
...
```{r}
#test the difference between prototypically big when the adjective is small and prototypically small objects when the adjective is big -- incompatible questions
